<template>
    <div v-if="this.customer">
        ...
        <span class="button is-smail btn-danger" v-on:click="deleteCustomer()">Delete</span>
    </div>
    <div v-else>
        <br />
...
export default {
    ...
    methods: {
        updateActive(status) {
            ...
            http.put("/customers/" + this.customer.id, data)
            .then(response => {
        ...
        deleteCustomer() {
            http.delete("/customers/" + this.customer.id)
            .then(response => {
                console.log(response.data);
